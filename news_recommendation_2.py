...
engine = sqlalchemy.create_engine('mysql+pymysql://root:@localhost:3306/section')
...
def predict_article(user, top_n=3):
    ...
    data = []
    print('\nTop {} recommendations for user {}:'.format(top_n, user))
    for i, title in enumerate(titles[0, :top_n].numpy()):
        rec = title.decode("utf-8")
        url = df.set_index('Title').loc[rec, 'Url']
        # print('{}. {} - {}'.format(i+1, rec, url))
        data.append(rec)    
...
# #Memanggil model yang sudah disimpan
# loaded_model = ArticleModel(rating_weight=1.0, retrieval_weight=1.0)
